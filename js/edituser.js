...
function editname() {
    ...
    Swal.fire({
        ...
        html: pnameValue + fnameValue + " " + lnameValue + " (" + positionValue + ")",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            ...
            var dataperson = `?id=${localStorage.getItem('uuid')}&pname=${encodeURIComponent(pnameValue)}&fname=${encodeURIComponent(fnameValue)}&lname=${encodeURIComponent(lnameValue)}&position=${encodeURIComponent(positionValue)}`;
...
function editwork() {
    ...
    Swal.fire({
        title: 'ยืนยันการแก้ไข.!',
        html: csubValue + " : " + xsubValue + " สังกัด " + cmainValue + " : " + xmainValue,
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#007bff',
        cancelButtonColor: '#d33',
        ...
        if (result.isConfirmed) {
            ...
            var dataperson = `?id=${localStorage.getItem('uuid')}&xmain=${xmainValue}&xsub=${xsubValue}&cmain=${cmainValue}&csub=${csubValue}&lat=${latValue}&long=${longValue}&db1=${db1Value}`;
            fetch(urlperson + dataperson)
                .then(response => {
                    if (!response.ok) {
                        throw new Error(`HTTP error! Status: ${response.status}`);
...
function editpic() {
    ...
    if (!yourpic || yourpic.trim() === '') {
        ...
        Swal.fire({
            title: 'ไม่พบรูปภาพ จากไลน์ของทาน!',
            text: 'กรุณาทำการ Logout แล้วทำการ Login ใหม่อีกครั้ง หรือเว็บไม่สามารถเข้าถึงรูปภาพ Profile Line ของท่านได้',
            icon: 'error'
        });
...
function sentnotify(buttonnotify,tokenValue) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            var dataperson = `?id=${localStorage.getItem('uuid')}&notify=${buttonnotify}&token=${tokenValue}`;
            fetch(urlperson + dataperson)
                .then(response => {
                    if (!response.ok) {
                        throw new Error(`HTTP error! Status: ${response.status}`);
